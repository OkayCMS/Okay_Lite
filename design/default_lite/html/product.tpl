...
<div class="border-b-1-info">
	<div class="container" itemscope itemtype="http://schema.org/Product">
		...
		<div class="row fn-transfer">
			{if $product->image}
				<div class="col-lg-5">
					<a class="fn-zoom okaycmslite btn-block relative border-a-1-info text-xs-center" href="{$product->image->filename|resize:800:600:w}" rel="group">
						{* Большое фото товара *}
					...
					{if $product->images|count > 1}
						<div class="row m-y-2 fn-slick-images okaycmslite">
							{* cut удаляет первую фотографию, если нужно начать 2-й - пишем cut:2 и тд *}
							{foreach $product->images|cut as $i=>$image}
								<div class="col-xs-4 col-lg-3">
									<a class="fn-zoom okaycmslite btn-block border-a-1-info text-xs-center product-images" href="{$image->filename|resize:800:600:w}" rel="group">
            ...
            {else}
                <div class="col-lg-5">
                    <a class="fn-zoom okaycmslite btn-block relative border-a-1-info text-xs-center" href="design/{$settings->theme}/images/no_image.png" rel="group">
                        {* Большое фото товара *}
			...
			<div class="col-lg-7 fn-product" itemprop="offers" itemscope="" itemtype="http://schema.org/Offer">
				<form class="fn-variants okaycmslite row" action="/{$lang_link}cart">
					<div class="col-lg-6">
						{* Цена *}
						...
						{* Варианты товара *}
						<select name="variant" class="fn-variant okaycmslite form-control c-select m-t-1 m-b-1-md_down{if $product->variants|count < 2} hidden-xs-up{/if}">
							{foreach $product->variants as $v}
								<option value="{$v->id}" data-price="{$v->price|convert}" data-stock="{$v->stock}"{if $v->compare_price > 0} data-cprice="{$v->compare_price|convert}"{/if}{if $v->sku} data-sku="{$v->sku|escape}"{/if}{if $v@first} selected{/if}>{if $v->name}{$v->name}{else}{$product->name|escape}{/if}</option>
							{/foreach}
						</select>
					</div>
					<div class="col-lg-6">
						...
						<div class="fn-product-amount fn-is_stock okaycmslite text-xs-center text-md-left{if $product->variant->stock < 1} hidden-xs-up{/if}">
							{* Кол-во товаров *}
